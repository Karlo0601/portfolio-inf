...
class Users {
  ...
  public function send_main_when_user_role_changes( $user_id, $new_role ) {
    ...
    $subject   = sprintf( esc_html__( 'Role changed: %s', 'portfolio-inf' ), $site_url );
    $message   = sprintf( esc_html__( 'Hello %1$s, your role has changed on %2$s. Congratulations, you are now an %3$s.', 'portfolio-inf' ), $user_info->display_name, $site_url, $new_role );
